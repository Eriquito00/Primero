...
create_room.addEventListener("click", () => {
  createRoom(username.value);
});
submit.addEventListener("click", () => {
  enterRoom(username.value, code.value);
});
...
function createRoom(username: string) {
  if (validateInfo(username)) {
    ...
    alert(
      "El username tiene que ser de 8 caracteres o mas y el codigo de 6 caracteres.",
    );
  }
}
...
function enterRoom(username: string, code: string) {
  if (validateInfo(username, code)) {
    ...
    alert(
      "El nombre debe tener al menos 2 caracteres y el código de la sala 6 caracteres.",
    );
  }
}
...
function validateInfo(username: string, code?: string): boolean {
  if (code === undefined) return username.trim().length >= 2;
  return username.trim().length >= 2 && code.trim().length === 6;
}
